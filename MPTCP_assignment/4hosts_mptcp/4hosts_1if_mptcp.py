#!/usr/bin/env python
#****************************************************************************#
#             MPTCP 4-hosts 1-i/f Average Throughput testing                 #
#****************************************************************************#
...
def fn(h, server, i):
    ...
    h.cmd('iperf -n 2G -c '+ server.IP() +' >> iperf_mptcp_1if_client'+ str(i) +'_log.txt &')

    #give time to print iperf output
    time.sleep(test_duration/2.0)
    print "\niperf client"+ str(i) +" response:"
    h.cmd('cat iperf_mptcp_1if_client'+ str(i) +'_log.txt')
